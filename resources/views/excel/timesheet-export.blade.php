<table border="1" style="width: 100%; border-collapse: collapse;">
    ...
    <tbody>
        ...
        @endphp
        @foreach ($data_kronos as $data_perOracle)
            @php
                $emp_amount = 0;
                $emp_eti_bonus = 0;
                $emp_amount_total = 0;
            @endphp
            @foreach ($data_perOracle as $data_perParentID)
                @foreach ($data_perParentID['data'] as $data_perEmployee)
                    @foreach ($data_perEmployee as $row)
                        <tr>
                ...
                <tr>
                    ...
                    @endfor
                    @foreach ($data_perParentID['total_overtime_hours'] as $total)
                        <td style="background-color: #d5d5d5; font-weight: bold; border: 2px solid black;"></td>
                        <td style="background-color: #d5d5d5; font-weight: bold; border: 2px solid black;"></td>
                        <td style="background-color: #d5d5d5; font-weight: bold; border: 2px solid black;"></td>
                    @endforeach
                    <td data-format="#,##0.00"
                        ...
                        {{ $data_perParentID['actual_hours_total'] }}
                    </td>
                    <td data-format="#,##0.00"
                        style="background-color: #d5d5d5; font-weight: bold; border: 2px solid black;">
                        {{ $data_perParentID['paid_hours_total'] }}
                    </td>
                    <td style="background-color: #d5d5d5; font-weight: bold; border: 2px solid black;"></td>
                    <td data-format="#,##0.00"
                        style="background-color: #d5d5d5; font-weight: bold; border: 2px solid black;">
                ...
                @php
                    ...
                    $super_actual_hours_total = bcadd(
                        $super_actual_hours_total,
                        $data_perParentID['actual_hours_total'],
                        6,
                    );
                    $super_paid_hours_total = bcadd($super_paid_hours_total, $data_perParentID['paid_hours_total'], 6);
                @endphp
            @endforeach
        @endforeach

        @foreach ($data_nk as $data_perOracle)
            @php
                $emp_amount = 0;
                $emp_eti_bonus = 0;
                $emp_amount_total = 0;
    ...
    @if (!empty($dailyRates))
        <tbody>
            ...
            @foreach ($dailyRates as $dailyRate)
                ...
                <tr>
                    ...
                    @foreach ($dailyRate->dailyDetails as $date)
                        ...
                        <td style="border: 2px solid black"></td>
                    @endforeach
                    <td data-format="#,##0.00" style="border: 2px solid black">{{ $dailyRate->work_hours_total }}
                    </td>
                    <td data-format="#,##0.00" style="border: 2px solid black">{{ $dailyRate->invoice_hours_total }}
                    </td>
            ...
            <tr>
                ...
                @endforeach
                <td data-format="#,##0.00"
                    style="border: 2px solid black; background-color: #d5d5d5; font-weight: bold;">
                    {{ $total_work_hours }}</td>
                <td data-format="#,##0.00"
                    style="border: 2px solid black; background-color: #d5d5d5; font-weight: bold;">
