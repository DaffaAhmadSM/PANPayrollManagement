<table border="1" style="width: 100%; border-collapse: collapse;">
    <thead>
        <tr>
            <th style="border: 2px solid black;" rowspan="2" align="center" valign="center" width="25">Kronos Job
                Charge</th>
            <th style="border: 2px solid black;" rowspan="2" align="center" valign="center" width="25">Parent ID
            </th>
            <th style="border: 2px solid black;" rowspan="2" align="center" valign="center" width="25">Oracle Job
                Charge</th>
            <th style="border: 2px solid black;" rowspan="2" align="center" valign="center" width="25">Employee
                Name</th>
            <th style="border: 2px solid black;" rowspan="2" align="center" valign="center" width="25">Emp.</th>
            <th style="border: 2px solid black;" rowspan="2" align="center" valign="center" width="25">
                Classification</th>
            <th style="border: 2px solid black;" rowspan="2" align="center" valign="center" width="25">Service
                Order No.</th>
            @foreach ($days as $day)
                @if ($day['is_holiday'])
                    <th colspan="3" align="center" valign="center"
                        style="background-color: #f29a6e; border: 2px solid black;">{{ $day['date'] }}</th>
                @else
                    <th style="border: 2px solid black;" colspan="3" align="center" valign="center">
                        {{ $day['date'] }}</th>
                @endif
            @endforeach
            ...
            <th style="border: 2px solid black;" rowspan="2" align="center">ETI Bonus
                {{ $temptimesheet['eti_bonus_percentage'] }}%</th>
        ...
        @foreach ($output as $key => $depart)
            ...
            @foreach ($depart as $data_output)
                ...
                @foreach ($data_output['data'] as $row)
                    <tr>
                        ...
                        @foreach ($row['dates'] as $overtime)
                            @for ($i = 0; $i < 3; $i++)
                                @if (!$overtime['is_holiday'] && $i == 0)
                                    @if ($overtime['basic_hours'] > 0)
                                        <td data-format="0.0">{{ $overtime['basic_hours'] }}</td>
                                    @else
                                        <td></td>
                                    @endif
                                    @continue
                                ...
                                @if (!$overtime['is_holiday'])
                                    @if (isset($overtime['overtime_timesheet'][$i - 1]) && $overtime['overtime_timesheet'][$i - 1] != 0)
                                        <td data-format="0.0">{{ $overtime['overtime_timesheet'][$i - 1] }}</td>
                                    @else
                                        <td></td>
                                    @endif
                                @else
                                    @if (isset($overtime['overtime_timesheet'][$i]) && $overtime['overtime_timesheet'][$i] != 0)
                                        <td data-format="0.0">{{ $overtime['overtime_timesheet'][$i] }}</td>
                                    @else
                                        <td></td>
                                    @endif
                                @endif
                            @endfor
                        @endforeach
                        <td data-format="0.0">{{ $row['actual_hours_total'] }}</td>
                        <td data-format="0.0">{{ $row['paid_hours_total'] }}</td>
                        ...
                        @php
                            ...
                            $eti_bonus = bcdiv(bcmul($amount, $temptimesheet['eti_bonus_percentage'], 2), 100, 2);
                ...
                <tr>
                    ...
                    @endforeach
                    <td data-format="0.00" style="background-color: #d5d5d5;">{{ $data_output['actual_hours_total'] }}
                    </td>
                    <td data-format="0.00" style="background-color: #d5d5d5;">{{ $data_output['paid_hours_total'] }}
                    </td>
                    <td style="background-color: #d5d5d5;"></td>
