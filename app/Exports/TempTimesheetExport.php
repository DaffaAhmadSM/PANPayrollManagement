...
class TempTimesheetExport implements
    FromView,
    ShouldQueue,
    ShouldAutoSize,
    WithStyles
{
    ...
    public function view(): View
    {
        try {
            ...
            $employee_rates = EmployeeRate::where(
                "random_string",
                $temptimesheet->rate_id
            )->first();
            ...
            $output = $data
                ->groupBy(["no", "Kronos_job_number", "oracle_job_number", "parent_id"])
                ->map(function ($byKronos) use (&$holiday, &$employee_rate_details, &$employee_department) {
                    ...
                    $employee = $byKronos->first()->first()->first()->first();
                    $dep =
                        $employee_department
                    ...
                    $data = $byKronos
                        ->map(function ($byParentId) use (&$holiday, &$total, &$employee_rate_details, &$employee) {
                            return $byParentId
                                ->map(function ($byOracle) use (&$holiday, &$total, &$employee_rate_details, &$employee) {
                                    return $byOracle
                                        ->map(function ($byEmployee) use (&$holiday, &$total, &$employee_rate_details, &$employee) {
